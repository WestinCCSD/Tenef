...
# override flags for dungeon entrace
export(bool) var override_entrance = true
...
func _load() -> void:
	...
	if dat.world.has("structures"):
		for struct in dat.world.structures:
			_load_structure(struct)
	...
	entrance.override_flag = override_entrance
	get_parent().call_deferred("add_child", entrance)
	


func _load_structure(structure : Array) -> void:
	# structure[0] path to scene
	# structure[1] vector2 for position
	# structure[2] custom data
	var instance = load(structure[0]).instance()
	instance.position = Vector2(structure[1][0], structure[1][1])
	if get_tree().has_group("overworld"):
		var overworld = get_tree().get_nodes_in_group("overworld")[0]
		overworld.call_deferred("add_child", instance)
		instance.call_deferred("_load_data", structure[2])
...
#						AI._add_outline(AI._get_outline_from_tile(Vector2(x, y), cell_size))
	...
	entrance_instance.override_flag = override_entrance
	...
	# deprecated code, may be removed or kept for looking back ons :)
	if generate_exit and false:
