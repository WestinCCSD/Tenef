...
func _process(_delta):
	if Input.is_action_just_pressed("inventory_open") and !(get_tree().has_group("storageui")) and !(PlayerInventory.player.typing_mode):
...
func _show() -> void:
	...
	get_tree().call_group("quickboxes", "_hide")
	_change_quickbox_visibility(true)
	$RecipeBrowser.show()
	$ItemboxDelete.show()
	$Background.show()
	$Items.show()
	...
	$Chestbox.show()
	$Feetbox.show()
	get_tree().call_group("trinketboxes", "show")
	$Gold.position = Vector2(96, 1050)

func _hide() -> void:
	is_visible = false
	get_tree().call_group("itemboxes", "_hide_box")
	get_tree().call_group("itemboxes", "_reset_size")
	get_tree().call_group("iteminfo", "_delete")
	get_tree().call_group("quickboxes", "_show")
	_change_quickbox_visibility(false)
	$RecipeBrowser.hide()
	$ItemboxDelete.hide()
	$Background.hide()
	$Items.hide()
	...
	$Chestbox.hide()
	$Feetbox.hide()
	get_tree().call_group("trinketboxes", "hide")
	get_tree().call_group("equipboxes", "_update_self")
	$QuickBoxE.show()
	$QuickBoxQ.show()
	$Gold.position = Vector2(48, 352)


func _change_quickbox_visibility(pvisible : bool) -> void:
	if pvisible:
		$QuickBoxE.show()
		$QuickBoxQ.show()
	else:
...
func _create_item_boxes() -> void:
	$InventoryInstance._create_item_boxes(true, null, true)
	_hide()
	_update_inventory()
...
func _update_inventory_size() -> void:
	...
	_create_item_boxes()
	$SettingsMenuUI.raise()


# code that's executed when any item box is clicked
func _on_itembox_clicked(index : int, box : Node, _inventory : Inventory) -> void:
	if holding_item:
