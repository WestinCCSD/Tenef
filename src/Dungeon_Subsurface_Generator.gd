...
func _init() -> void:
	structure_spawn_rate = 1.0
	passable_structures = [
		Structure.new("res://scenes/structures/BlankStruct.tscn", 0, 8, false),
		Structure.new("res://scenes/structures/Structure_Root.tscn", 0, 5),
		Structure.new("res://scenes/structures/Structure_Tile_Root.tscn", 0, 1)
	]
	topmost_structures = [
		Structure.new("res://scenes/structures/BlankStruct.tscn", 0, 135, false),
		Structure.new("res://scenes/structures/Structure_Stone_Dirt.tscn", 0, 9, false),
		Structure.new("res://scenes/structures/Structure_Boulder.tscn", 0, 3),
		Structure.new("res://scenes/structures/Structure_Iron.tscn", 0, 1),
		Structure.new("res://scenes/structures/Crate.tscn", 0, 4),
	]
	impassable_structures = [
		Structure.new("res://scenes/structures/BlankStruct.tscn", 0, 1, false)
	]
	clustergroups = [
		load("res://scenes/enemies/EnemyDinosaur.tscn"),
		load("res://scenes/enemies/EvilCoconut.tscn")
	]
...
func _find_passable_structure() -> Structure:
	if randf() <= 0.3:
		return passable_structures[1] 
	else:
		return passable_structures[2]
...
func _generate_circles() -> void:
	...
	for _i in range(rand_range(4, 7 + Game.floor_number)):
		var rand_position : Vector2 = Vector2.ZERO
		var minx : int = 4 - Game.floor_number
		minx = clamp(minx, 1, 99)
		var maxx : int = 17 + Game.floor_number
		var rand_rad : int = rand_range(4, 8)
		rand_position = Vector2(rand_rad + rand_range(minx, maxx), rand_rad + rand_range(minx, maxx))
...
func _find_dungeon_spawn(_passable_spaces : PoolVector2Array, topmost_spaces : PoolVector2Array) -> Vector2:
