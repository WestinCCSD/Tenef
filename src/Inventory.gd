...
# deletes item history and recalculates it
# should be used sparingly
func _recalculate_history() -> void:
	storage_history.clear()
	for item in storage:
		if !storage_history.has(item.item_name):
			storage_history[item.item_name] = 0
		storage_history[item.item_name] += item.amount
...
func _to_arr() -> Array:
	var arr = []
	for i in storage.size():
		arr.push_back([storage[i].item_name, storage[i].amount])
	return arr


func _from_arr(arr : Array) -> void:
	if arr.size() != storage.size():
		print("Size difference when loading inventory from array!")
	
	# 0 = name
	# 1 = amount
	for i in storage.size():
		var entry = arr[i]
		var item = ItemLookup._get_as_item(entry[0])
		item.amount = entry[1]
		storage[i] = item
