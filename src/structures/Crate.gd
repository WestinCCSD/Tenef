...
func _spawn_loot() -> void:
	var rand_index = randi() % loot_pool.size()
	var entry = loot_pool[rand_index]
	if entry.hash() == "Gold".hash():
		var goldamount = floor(rand_range(3.0, 7.0))
		for i in range(goldamount + 1):
			_spawn_gold(global_position)
		return
	var item = ItemLookup._get_as_item(entry)
	item.amount = 1
	PlayerInventory.player._spawn_dropped_item(item, global_position)
