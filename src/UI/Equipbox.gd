...
func _ready() -> void:
	...
	elif type == armor_type.trinket:
		$BaseSprite.texture = load("res://assets/art/ui/item_trinket.png")
	item = Item.new()
	$Anims.play("box_zoom", -1, -1.0, true)
	remove_from_group("itemboxes")
	add_to_group("equipboxes")
	if type == armor_type.trinket:
		add_to_group("trinketboxes")
...
func _on_mouse_entered():
	if pickable:
		$Anims.play("box_zoom")
		print(PlayerInventory.equipment)
